...
const {TwitterFollowersAdapter} = require('../util/Adapter');
...
class TwitterFollowersTrends extends React.Component {
    constructor(props) {
        super(props);

        const {originUrl} = props;
        this.adapter = new TwitterFollowersAdapter({originUrl});
    ...
    componentDidMount() {
        return (async()=>{
            this.trendsList = (await this.adapter.getTrendsList()).adjustTrendsByDate2();
            this.trendsList.rank({key:"latest"}).addPastRank({key:"latest"});
...
Object.assign(module.exports,{TwitterFollowersTrends});